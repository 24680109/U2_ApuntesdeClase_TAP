...
def generar_grafica_dispersion():
    ...
    y = [random.randint(30, 50) for _ in range(20)]
    ...
    plt.tight_layout()
    return fig
def generar_grafica_barras2():
    productos = ['A', 'B', 'C', 'D']
    ventas = [30, 45, 60, 15]
    fig, ax = plt.subplots(figsize=(4, 3))
    ax.bar(productos, ventas, color="red")
    ax.set_title("Ventas", fontsize=10, weight='bold')
    plt.tight_layout()
    return fig


def main(page: ft.Page):
    page.title = "Dashboard FNP - Gráfica 3"
    ...
    fig_4 = generar_grafica_barras2() # Reutilizamos la función de barras
    ...
    plt.close(fig_4) # Siempre cerramos la figura para liberar memoria
